...
class ELMRegressor():
    ...
    def Tratamento_dados(self, serie, divisao, lags):
        ...
        particao = Particionar_series(serie, divisao, lags, norm=True)
...
def main():
    ...
    #load da serie
    import pandas as pd
    dataset = pd.read_csv("WINJ19_M5_201903061300_201904051750.csv", header=None, sep='\t')[5][1:].values.astype(np.float)
    
    # treinando da forma convencional
    ELM = ELMRegressor(10)
    ELM.Tratamento_dados(dataset, [0.6, 0.2, 0.2], 5)
    ...
    plt.plot(ELM.train_saidas, label="Train")
    plt.plot(previsao, label="Prediction")
    plt.legend()
    plt.show()
    
    # realizando a previsao no teste da forma convencional
    previsao = ELM.Predizer(ELM.teste_entradas)
    MAE = mean_absolute_error(ELM.teste_saidas, previsao)
    print('ELM - Test MAE: ', MAE)
    plt.plot(ELM.teste_saidas, label="Test")
    plt.plot(previsao, label="Prediction")
    plt.legend()
    plt.show()
